document.addEventListener('DOMContentLoaded', function () { (() => {
	if (document.querySelectorAll('[ct_data*="eighteenimg"]')) {
		const imageBlock = document.querySelectorAll('.aifEighteen__container')
		imageBlock.forEach((elem) => {
			const changeImg = () => {
				if (getComputedStyle(elem.children[0]).getPropertyValue('display') === 'block') {
